...
describe('CmuxPlanAgents', () => {
  it('the_call_it_makes_names_the_ticket_and_cuts_it_in_the_directory_it_was_given', async () => {
    ...
    expect(cmux.calls).toEqual([[
      ...
      '--cwd', CmuxDouble.CWD,
      '--command', 'echo "plan agent up for MO_SHOP-42 on issue #7"',
    ]])
  })
  ...
  it('the_handle_cmux_prints_is_what_comes_back_so_the_caller_can_reach_the_agent_later', async () => {
    ...
    expect(await cmux.launchFor('ABC-123')).toBe('workspace:4')
  })
  ...
  it('the_notice_cmux_prints_before_the_handle_does_not_get_mistaken_for_one', async () => {
    ...
    expect(await cmux.launchFor('ABC-123')).toBe('workspace:12')
  })

  it('output_with_no_handle_in_it_raises_instead_of_handing_back_something_unusable', async () => {
    const cmux = new CmuxDouble('OK\n')

    await expect(cmux.launchFor('ABC-123')).rejects.toThrow(/did not name the workspace/)
  })
  ...
  it('a_cmux_that_refuses_the_call_arrives_typed_so_the_caller_can_tell_it_from_a_crash', async () => {
    ...
    const refusal = await cmux.launchFor('ABC-123').catch((cause) => cause)
  ...
  it('cmux_answering_something_unreadable_is_told_apart_from_cmux_refusing_the_call', async () => {
    const unreadable = await new CmuxDouble('OK\n').launchFor('ABC-123').catch((cause) => cause)
    const refused = await CmuxDouble.refusing('Access denied').launchFor('ABC-123').catch((c) => c)
