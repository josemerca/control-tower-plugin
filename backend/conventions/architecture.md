...
Applies to: **new modules and moves**. The three layers, the direction of
...
document only adds what the backend decided on top.
...
This is `mo.catalog.api`'s partition (`value_objects/`, `repositories/`,
`actions/`, `queries/`), adopted when a flat `domain/` stopped reading.

## When two types share a module

The repository's rule decides ("delete the main concept: is the other left
with no consumer and no sense of its own?"), and these are the calls already
made under it, kept so they are not relitigated:
...
- **A test double is not a consumer and tests are not a layer.** Neither grants
  a type a module of its own.
