...
export const GATES = {
  ...
  plan: {
    ...
    kickoff: 'GATE HUMANO `plan` (implicado por defecto en TODO slice, salvo renuncia `!plan` en el spec): con el plan del slice escrito, validado con --check-plan y commiteado, publícalo como comentario del issue y PARA — no implementes nada hasta que un humano conteste el go en un comentario de ese issue. El go es `-OK <nonce>`, y el nonce lo sorteó la coordinadora al despacharte: NO está en este kickoff, ni en el issue, ni en tu worktree, y no puedes fabricarlo — `dispatch-check --release` se niega (exit 9) sin un go válido, así que saltarte este gate no te deja entregar, sólo te deja rehacer el trabajo. Un vigilante que lanzó la coordinadora está mirando el issue y te teclea la línea cuando el go llegue, así que PARAR de verdad es lo correcto: no sondees tú el issue ni te des el gate por cumplido. En el comentario que publicas NO escribas `<nonce>` como si fuera el formato entero —quien lo lea se queda sin saber de dónde sale y prueba el `-OK` pelado, que no arranca nada—: di que el go es `-OK` seguido del nonce que `/ct-next` imprimió al despachar, y que si se perdió lo reemite quien despachó. No lo cierras tú: lo cierra quien revisa el plan.',
