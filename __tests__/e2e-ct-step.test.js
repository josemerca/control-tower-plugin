...
describe('ct-step e2e', () => {
  ...
  it('fuera de su paso: exit 9 y dice cuál toca', () => {
    ...
    try {
      const run = JSON.parse(readFileSync(join(dir, '.agent', 'run-4.json'), 'utf8'))
      // `task: 2` y no `task: 1` (que sería el `task` real de este fixture):
      // hay UN commit desde `baseSha` (worktreeEnE2e comitea una sola vez, la
      // "tarea 1"), así que fuera del paso `e2e` la invariante exige
      // `task - 1 === 1`, o sea `task: 2`. Con `task: 1` el propio guard de
      // PRECONDITION dispara antes de llegar a `exigirPaso` — correcto para
      // ESE control, pero no es lo que este test quiere ejercitar.
      writeFileSync(join(dir, '.agent', 'run-4.json'), JSON.stringify({ ...run, step: 'implement', task: 2 }))
      const r = step(dir, ['e2e', informe(dir, VERDE)])
