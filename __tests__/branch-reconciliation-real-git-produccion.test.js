...
const aSliceBranchWhoseBaseIsAboutToMove = ({ files, onTheSlice, onTheBase }) => {
...
const aBranchAlreadyMergedByHand = ({ resolution }) => {
  const dir = aSliceBranchWhoseBaseIsAboutToMove({
...
describe('BranchReconciliation contra git real, con la forma que tiene producción', () => {
  it('a_base_that_brought_a_clean_file_alongside_the_conflicted_one_still_reaches_resolved_with_a_real_merge_commit', () => {
    ...
    try {
      const first = new BranchReconciliation({ git: gitPort(dir) }).merge({ baseBranch: 'main' })
      ...
      const second = new BranchReconciliation({ git: gitPort(dir) }).conclude()
  ...
  it('a_merge_someone_else_committed_with_markers_inside_is_caught_post_hoc_instead_of_reported_as_up_to_date', () => {
    const dir = aBranchAlreadyMergedByHand({
      resolution: '<<<<<<< HEAD\nthe slice line\n=======\nthe base line\n>>>>>>> origin/main\n',
    })
    try {
      const round = new BranchReconciliation({ git: gitPort(dir) }).merge({ baseBranch: 'main' })
  ...
  it('a_merge_someone_else_committed_and_resolved_properly_is_up_to_date_and_not_accused', () => {
    const dir = aBranchAlreadyMergedByHand({ resolution: 'the slice line\nthe base line\n' })
    try {
      const round = new BranchReconciliation({ git: gitPort(dir) }).merge({ baseBranch: 'main' })
