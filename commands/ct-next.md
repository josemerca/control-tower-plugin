...
**Advertencia honesta sobre el claim concurrente (T11):** el lock que usa `dispatch-check.mjs` para reclamar un issue (`status:ready` → `status:in-progress`) es labels de GitHub, sin compare-and-swap. Está reproducido y verificado (no solo sospechado) que dos dispatchers lanzados casi a la vez pueden reclamar el mismo token compartido (`area:`/`touches:`) y arrancar los dos — ver `scripts/experiments/ac6-race2-deterministic.sh` y `task-11-report.md` para la evidencia (6/6 rondas con doble claim real, confirmado contra el estado de los labels en GitHub). No hay ninguna espera ni reintento que cierre este hueco hoy. **La mitigación real es operativa: no lances dos `/ct-next` (ni dos dispatchers) al mismo tiempo contra el mismo repo.** El plan es migrar el claim a una primitiva atómica real (test-and-set vía `git refs`); hasta entonces, esta es la única garantía que existe.
