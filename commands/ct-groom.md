...
### `Señal` — la señal de observabilidad del slice (Slice 10)

`Señal` *(opcional)* declara la **señal de observabilidad** que el slice promete: qué métrica, log o evento tiene que emitir su código de producción cuando llegue a producción (p.ej. "métrica `backfill_progress` con label `estado`"). Texto libre de una sola pieza, como `Protegido`: la coma no separa nada.

Si el slice no tiene nada observable que prometer, se declara la **exención razonada**: `N/A — <razón>` (el mismo idioma que la Global verification de un plan). Una exención **sin razón** (`N/A` a secas, o con el separador y nada detrás) **aborta con exit 2** — a diferencia de un `Tipo` desconocido, que solo avisa: la razón es lo que un humano aprueba en el groom y lo que el juez de slice cita como `no-aplica`; una exención que nadie puede leer es una señal sin declarar disfrazada de decisión.

A dónde llega: sección **`## Señal de observabilidad`** del cuerpo del issue (verbatim de la celda, solo cuando hay algo que decir — señal o exención), de ahí al campo `senal:` del **`.agent/SLICE.md`** del worktree en el despacho (lo siembra `/ct-next`; la ausencia se declara, nunca se omite), de ahí a la primera sección **`## Señal`** del paquete del juez de slice (la pega `ct-step`, leída del disco y sin ningún agente en medio), donde el ítem **`observabilidad`** de `ct-slice-judge` la mide contra el diff acumulado: que lo prometido lo emita código de producción, instrumentado como ya instrumenta este repo, sin labels de cardinalidad ilimitada. El kickoff del agente además la nombra cuando el issue la declara.

Celda vacía o con marcador de "sin valor" significa *no lo he pensado* — no es una exención: el juez de slice mide su ítem como **`sin-vara`**, y esa cuenta viaja en la telemetría del epic (`rubric_sin_vara`). Si el spec no trae la columna entera, `/ct-groom` avisa por consecuencia: todos los slices del epic saldrán `sin-vara` en ese ítem.
...
**No puede contener nada que la corte antes de tiempo**: ni subcabeceras de nivel 3 o más (`###`, `####`…) ni un comentario HTML autocontenido (uno que abre y cierra `<!-- … -->` en la misma línea — el mismo criterio de "fin de sección" que ya rige el resto de secciones del cuerpo del issue, ver "El fin de una sección…" más abajo). Esas dos cosas, si aparecen dentro, sí truncan: `/ct-groom` avisa nombrando la línea que corta y **no emite la sección en ningún issue**.

**Tampoco puede dejar un delimitador sin cerrar**: una valla de código (` ``` `) o un comentario HTML (`<!--`) abiertos y sin cerrar hacen lo contrario que un truncamiento — esconden del escáner todo lo que viene detrás, así que la sección deja de terminar donde parece y se traga el resto del spec, la tabla de slices incluida. `/ct-groom` lo detecta y tampoco emite la sección. Una valla **bien cerrada** es contenido legítimo y no dispara nada.

Los cuatro casos en que la sección no se emite (ausente, vacía, truncada, con un delimitador sin cerrar) avisan **por separado**, porque cada uno se arregla de forma distinta — y no significan lo mismo para `--reconcile`: **ausente** o **vacía** dicen que el epic no tiene contexto común, y entonces la sección se retira del cuerpo de los issues que la tuvieran; **truncada** o **con un delimitador sin cerrar** dicen que no se ha podido leer un texto válido, y entonces no se toca nada — un error de formato en el spec no borra el contexto que ya llevan los N issues. Una cabecera de **nivel 1 o 2** detrás del texto, en cambio, **no** es un problema: es el final normal de la sección (el mismo con el que termina cualquier otra sección del spec) y pasa **en silencio** — sin aviso, con el contexto emitiéndose con normalidad hasta ahí. Usa negritas o listas dentro, nunca cabeceras.

### `## Decisiones congeladas` — sección del núcleo de la plantilla del spec

Como el resto de secciones del spec, esta la gobierna la plantilla (`_TEMPLATE-execution-spec.md`, el núcleo); igual que `## Contexto del epic`, es opcional en lo que a `/ct-groom` respecta y se localiza **por el texto de su cabecera, no por su número de sección** (fuera de la tabla). Su contenido se copia al cuerpo de todos los issues del epic —para que el agente que implementa un slice tenga delante las decisiones del epic que su trabajo debe respetar, y las vuelque en `## 2. Closed decisions` de su plan—, y `--reconcile` lo mantiene al día si el spec cambia. **Diferencia con el contexto del epic:** de cada línea se quita la procedencia (`*(Procedencia: …)*`, en una sola línea) al proyectar — es meta para quien congela, no para quien ejecuta; si el recorte no la pilla, la línea viaja tal cual y `/ct-groom` lo avisa. Cada decisión es un bullet (`- **D-N · …** — …`); no metas cabeceras (`###`) dentro, o la sección no se emite (mismo guardarraíl que el contexto del epic). Un spec sin esta sección es válido.
...
**Y aguanta que pegues ahí el cuerpo de otro issue**, que es justo lo que hace falta ("el slice #2 dejó montado esto"): ese texto trae las MISMAS cabeceras que cualquier issue del epic, porque las escribe el mismo generador. `--reconcile` trata todo lo que hay **entre `## Contexto heredado` y `## Acceptance criteria`** como tuyo y no escribe ni un byte ahí — ni en las cabeceras que pegues ni en la prosa que escribas alrededor. Lo que sí puede pasar es que se **rinda**: si la única `## Dependencias` (o el ancla `## Out of scope / Protected`) del cuerpo es la que pegaste, `--reconcile` no la reescribe **y tampoco añade una segunda copia más abajo** — el dispatcher obedece la PRIMERA, que sería la tuya. Lo dice por `stderr` con el remedio concreto, y cuenta para el exit code. La excepción es `## Contexto del epic`: si la que hay pegada es la única, la suya se **inserta delante** de tu sección, sin tocarla, y a partir de ahí converge.

Un detalle que conviene saber porque decide el alcance de todo lo anterior: el final de tu zona lo marca `## Acceptance criteria`. Si esa cabecera **no aparece exactamente una vez** en el cuerpo (alguien la renombró, la borró, o pegaste otra copia), no hay forma de saber dónde termina lo tuyo — y entonces `--reconcile` no escribe **nada** por detrás de `## Contexto heredado`, se rinde en voz alta y te dice que la restaures. Es deliberado: de los dos errores posibles, el caro es el que borra texto escrito a mano.

### El spec entra congelado o no entra (F32)

Antes incluso de mirar la tabla, dos comprobaciones sobre el spec ENTERO — la puerta de congelación, pre-registrada en F32 como «dos greps en la pasada que groom ya hace»:

- **`[NEEDS CLARIFICATION` sin resolver → exit 2.** Se admiten mientras el spec está en DRAFT; groom solo acepta specs congelados, y congelar con un hueco pendiente es inválido: se resuelve con quien decide o se aparca en `## Decisiones aparcadas`. El mensaje nombra cuántos y la línea del primero.
- **`## Hipótesis` ausente o vacía → exit 2.** Sin apuesta falsable no es un epic y no entra por groom (decisión 2026-08-07); el trabajo sin apuesta (mantenimiento, bugfixes) va como issues sueltos. Un comentario HTML residual de la plantilla no cuenta como contenido. La *calidad* de la hipótesis la juzga el humano en la congelación — groom solo mira presencia: más listo que su pre-registro sería otro instrumento.

Caen junto a los errores de tabla de abajo, JUNTOS en un único exit 2, antes de cualquier mutación y también bajo `--dry-run`. Esto documenta la puerta aquí y no en el bloque del contrato a propósito: el contrato gobierna la tabla y no crece sin quitar; las secciones del spec las gobierna su plantilla (`_TEMPLATE-execution-spec.md`).
...
- **`## Señal de observabilidad`** se compara solo con un **sí/no**, como **nota** (mismo trato que Descripción/Protegido: no cuenta para el exit code, y `--reconcile` no la reescribe — la señal que obedece el juez de slice es la que el issue tenía al despachar, igual que los gates);
...
- **`## Contexto del epic`**, también vía `--body` (splice quirúrgico de sección, igual que AC/Dependencias): si el spec trae texto y la sección no existe todavía en el issue (uno de antes de esta ronda), se inserta entera justo **antes de `## Contexto heredado`** (sin tocar un byte de su contenido) o, si esa sección no está, justo antes de `## Acceptance criteria` — el orden que fija el contrato es epic → heredado → criterios. Sin ninguna de las dos cabeceras no se inventa una posición: no se escribe nada y se dice por `nota:` (nunca cuenta para el `3`). Si el spec deja de traer la sección (o la trae vacía), se retira entera; si la trae pero malformada, no se toca nada (ver arriba). "De quién es el texto" no es lo que distingue esta sección de Descripción/Protegido (las tres las posee el spec) — lo que la distingue es que editarla a mano en un issue suelto es exactamente la divergencia que mantenerla al día existe para eliminar.
...
- Una sección conocida **duplicada**: solo la primera aparición se compara, y **ninguna se reescribe** mientras haya dos — `--reconcile` no decide cuál copia es la del plugin (una puede ser texto pegado dentro de `## Contexto heredado`), así que se rinde y lo dice. Las dos copias sobreviven intactas (se avisa, no se corrige solo — y para AC/Dependencias, cuenta para el exit code, ver arriba).
