...
Todo esto funciona también bajo **`--dry-run`** (con `--repo`; sin él no hay nada contra qué comparar): el mismo reporte, y con `--reconcile` además anuncia qué aplicaría (por categoría — título/milestone/labels/dependencias/AC — sin volcar el `--body` reconstruido entero) — pero nunca muta nada.

**Código de salida**: `0` si no hay divergencia (o si `--reconcile` la resolvió con éxito), `2` si la tabla §9 es inválida (como siempre), y **`3`** si queda divergencia sin reconciliar. `3` es deliberadamente distinto de `0` (no sería "silencio real") y de `2` (no es que la tabla esté rota) — un exit no-cero aquí sería tan malo como el silencio que esto corrige, pero en la dirección opuesta: entrenaría a cualquier script que solo mire "¿salió con error?" a tratar una divergencia meramente informativa como si el spec estuviera roto. Esto vale **igual bajo `--dry-run`** que en la corrida real sin `--reconcile` — ante la MISMA divergencia, los dos dan el MISMO `3` (dry-run nunca resuelve nada de verdad, así que la divergencia detectada sigue en pie al terminar). Y vale también tras un `--reconcile` con éxito: si lo ÚNICO que quedaba era divergencia de Descripción/Protegido (que `--reconcile` nunca toca, ver arriba), el proceso sigue saliendo con `3` — el código de salida no miente sobre si de verdad no queda nada por revisar.
