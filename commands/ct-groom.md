...
**No puede contener nada que corte la sección**: ni subcabeceras (`###`, `####`…) ni un comentario HTML autocontenido (uno que abre y cierra `<!-- … -->` en la misma línea — el mismo criterio de "fin de sección" que ya rige el resto de secciones del cuerpo del issue, ver "El fin de una sección…" más abajo). El reemplazo con el que `--reconcile` la mantiene al día termina en lo primero que corte la sección — cualquier cabecera ATX de cualquier nivel, o un comentario autocontenido — así que cualquiera de esas dos cosas ahí dentro dejaría el resto del texto huérfano. Si el spec la trae con alguna de las dos, `/ct-groom` avisa nombrando la línea que corta y **no emite la sección en ningún issue** — mismo trato (avisar y no emitir) que un spec sin la sección o con ella vacía; los tres casos avisan por separado, porque cada uno se arregla de forma distinta. Usa negritas o listas dentro, nunca cabeceras.
