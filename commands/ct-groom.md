...
Una cabecera conocida se reconoce por **igualdad exacta** (no por prefijo) — salvo `## Acceptance criteria`, que sí tolera un sufijo variable (`(EARS, 1:1 con tests)`, el único hueco legítimo). Un `## Dependencias externas (notas del equipo)` escrito por un humano, sobre otra cosa, NUNCA se confunde con la sección real de dependencias.
...
Una sección conocida **duplicada** en el body (copiar-pegar, un merge mal resuelto) se avisa siempre — pero con una distinción importante: duplicar `## Dependencias` o `## Acceptance criteria` **SÍ cuenta para el código de salida** (`divergencia:`), porque el dispatcher no distingue "la primera aparición" — obedece la unión de ambas copias, así que la duplicación cambia de verdad lo que pasa en el despacho. Duplicar `## Descripción`/`## Out of scope / Protected` sigue siendo solo cosmético (`nota:`, no cuenta).
