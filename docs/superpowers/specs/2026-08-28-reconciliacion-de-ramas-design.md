...
| Fichero | Cambio |
|---|---|
| `scripts/run-machine.js` | paso `reconcile`, `BLOCKED_RECONCILE`, `reconcileRetries: 2` |
| `scripts/ct-step.mjs` | verbo `reconcile`; `commitsDesde` pasa a `merge-base` + `--no-merges`; lee `base:` de `.agent/SLICE.md` con el `parseStateSafe` que ya usa para `epic:` y `senal:` |
| `scripts/dispatch-check.mjs` | `stateFilesIntroducedByBranch()` (`:552`) y `branchIntroducedFiles()` (`:586`) usan `merge-base`; `readFileAtBase()` (`:623`) se queda con `base_sha` |
