...
- **Los tests que este plan añade se escriben con la vara del propio repo**: nombre en inglés y **sin comentarios**. `plugin/__tests__/modulos-conformes.test.js` mide los ficheros del plugin contra `style.md`, y un bloque de test con comentarios explicativos o con el nombre en castellano lo pone en rojo. Donde un bloque de código de este plan lleve un comentario, el comentario es para quien lee el plan y no viaja al fichero.
