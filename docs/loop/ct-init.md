# `/ct-init` — referencia y diseño

> Texto movido íntegro desde `plugin/commands/ct-init.md` (sub-issue #93). El comando conserva la invocación, la tabla de códigos de salida, las instrucciones al agente en forma de lista y un enlace a este documento.
>
> **Actualización (#93):** el texto de abajo describe el contrato de la tabla de slices como una sección de `AGENTS.md`. Desde esta ronda el contrato vive en `docs/superpowers/CONTRATO-SLICES.md` del repo gobernado (lo escribe y versiona `/ct-init`, con la misma doctrina de versión, hashes «pristine», `--update-slices-contract` y `--force` que se describe aquí) y `AGENTS.md` conserva una sección corta —los mismos marcadores `<!-- ct-init:slices-contract -->`, que son la señal de repo gobernado que lee la puerta de commits— que enlaza al contrato. Un `AGENTS.md` bootstrapeado antes conserva el contrato entero hasta que se corre `--update-slices-contract`, que sustituye la sección por la corta si estaba sin editar.
